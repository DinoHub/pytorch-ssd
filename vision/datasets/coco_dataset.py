...
class COCODataset:
    ...
    def __init__(self, root, transform=None, target_transform=None, is_test=False, keep_difficult=False, label_file=None):
        ...
        Args:
            root: the root of the COCO dataset, the directory contains the following sub-directories:
                images, labels.
        """
        ...
        if is_test:
            image_sets_file = os.path.join(self.root, "5k.txt") 
        else:
            image_sets_file = os.path.join(self.root, "trainvalno5k.txt")
        self.ids = COCODataset._read_image_ids(image_sets_file)
        self.keep_difficult = keep_difficult

        # if the labels file exists, read in the class names
        label_file_name = os.path.join(self.root, "coco.names")
    ...
    def _extract_image_id(image_file_path):
        return image_file_path.rstrip().split('/')[-1].split('.')[0]
    ...
    def _read_image_ids(image_sets_file):
        ...
        with open(image_sets_file) as f:
            for image_file_path in f:
                ids.append( COCODataset._extract_image_id( image_file_path ) )
        return ids

    def _get_image_filepath(self, image_id):
        return os.path.join(self.root, 'images/{}/{}.jpg'.format( image_id.split('_')[1], image_id ))

    def _get_annotation_filepath(self, image_id):
        return os.path.join(self.root, 'labels/{}/{}.txt'.format( image_id.split('_')[1], image_id ))
...
if __name__ == '__main__':
    ...
    def unit_test2():
        root = '/media/dh/DATA4TB/Datasets/coco'
        image_id = 'COCO_val2014_000000023017'
        dataset = COCODataset( root )
